...
The local parent directory must be set to one's environment in most codes supplied, and discussed under "Code". A subfolder "data" must be created by the user in the diretory "Code" (so different from "Data"), relative to this code directory. The "Data" folder is one directory up from this code directory and has to exist and contain the relevant and used data files.

May 2026: Codes have been updated to produce pdf-output images. RatingsComparison.py, RainfallvsFlow.py and hTvsFEVvsSidecode.py files added under the Code folder.
